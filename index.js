...
class Client {
		...
		return new Promise(async (resolve, reject) => {
			try {
				const res = await wump(
					`https://bots.unboxingman.com/util/idlist.json`,
					{
						method: "GET",
					}
				).send();
	...
	async getStaffList() {
		return new Promise(async (resolve, reject) => {
			try {
				const res = await wump(
					`https://bots.unboxingman.com/util/StaffList.json`,
					{
						method: "GET",
					}
				).send();
	...
	async postServers(clientID, servers) {
		if (typeof servers !== "number")
			throw new TypeError("Server count must be a number");
		...
		return new Promise(async (resolve, reject) => {
			try {
				const res = await wump(
					`https://bots.unboxingman.com/api/post/servers?id=${clientID}`,
					{
						method: "POST",
						data: {
							Servers: servers,
...
class db {
	...
	async dbLogin() {
		//console.log(`yes`)
		if (typeof this.dbjs !== "object")
			throw new TypeError("db login must be a json");
	...
	async push(id, json) {
		...
		return new Promise(async (resolve, reject) => {
			let currentArr = await this.getArray(id);
			//  console.log(currentArr)
			if (Array.isArray(json)) currentArr = currentArr.concat(json);
			else currentArr.push(json);
